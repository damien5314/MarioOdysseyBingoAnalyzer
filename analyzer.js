// analyzer.js
// Analyzes the Bingosync board and posts finishes below the board
...
function analyze(board) {
    ...
    if (board.length === 5) {
        ...
        let worstKingdom;
        let currKingdom;
        let taskList = this.analyzerData.normal;
...
function postResultsToContainer(results) {
	let boardContainer = document.getElementsByClassName("board-container")[0];
	var boardAnalyzerResultsContainer = document.getElementById("board-analyzer-results");
	if (boardAnalyzerResultsContainer == null) {
	  // Container has not yet been initialized, need to add it
	  boardAnalyzerResultsContainer = document.createElement("div");
	  boardAnalyzerResultsContainer.id = "board-analyzer-results";
	  boardContainer.appendChild(boardAnalyzerResultsContainer);
	} else {
	  // Container is already initialized; clear all children and add new ones
	  boardAnalyzerResultsContainer.innerHTML = "";
	}
	...
	for (result of results) {
		...
		resultDiv.style.padding = ".75rem 1.25rem";
		resultDiv.style.marginBottom = "-1px";
		resultDiv.style.border = "1px solid rgba(0, 0, 0, .125)";
		resultDiv.style.color = colorForegroundForKingdom(result.kingdom);
		resultDiv.style.backgroundColor = colorBackgroundForKingdom(result.kingdom);
		resultDiv.innerHTML = `${result.row} ends in ${result.kingdom.name}`;
		boardAnalyzerResultsContainer.appendChild(resultDiv);
		// boardAnalyzerResultsContainer.appendChild(document.createElement("br"));
	}
}

function colorForegroundForKingdom(kingdom) {
    if (kingdom.index > 9) {
...
function colorBackgroundForKingdom(kingdom) {
    if (kingdom.index > 9) {
		// bootstrap .list-group-item-danger
		return "#f5c6cb";
    } else if (kingdom.index > 7) {
...
function onActionClicked() {
	...
	let board = parseBoardHtml();
	let results = analyze(board);
	postResultsToContainer(results);
}

browser.runtime.onMessage.addListener(onActionClicked)

console.log("Bingo Analyzer extension loaded");
